 ...
 * estructurales por iconos vectoriales coherentes y theme-aware
 * (heredan el color vía `currentColor`).
 *
 * Tres formas de uso:
 *
 *   1) Declarativo (markup estático) — recomendado para HTML fijo:
 *        <span class="icon" data-icon="layout-dashboard"></span>
 *      icons.js hidrata todos los [data-icon] al cargar el DOM.
 *
 *   2) Programático (dentro de template strings de JS):
 *        Icon('trophy', { size: 16, label: 'Ranking' })  → "<svg …>…</svg>"
 *
 *   3) Re-hidratar tras render dinámico (tablas, listas):
 *        Icons.hydrate(contenedorEl)
 *
 * Robustez: si el nombre no existe devuelve "" (no rompe el layout).
 * Sin estado interno. Compatible con el patrón ES5/UMD del proyecto.
 * ═══════════════════════════════════════════════════════════════ */
...
(function (global) {
  ...
  var PATHS = {
    ...
    "wand":
      '<path d="m3 21 9-9"/><path d="M15 4V2"/><path d="M15 16v-2"/><path d="M8 9h2"/><path d="M20 9h2"/><path d="M17.8 11.8 19 13"/><path d="M15 9h.01"/><path d="M17.8 6.2 19 5"/><path d="m3 21 9-9"/><path d="M12.2 6.2 11 5"/>',
    "calendar":
  ...
  function svg(name, opts) {
    opts = opts || {};
    var body = PATHS[name];
    if (!body) return ""; // icono desconocido → no rompe nada
    var size = opts.size || 20;
    var cls = "lucide" + (opts.className ? " " + opts.className : "");
  ...
  // Reemplaza el contenido de cada [data-icon] por su SVG. Idempotente.
  function hydrate(root) {
    ...
    for (var i = 0; i < nodes.length; i++) {
      ...
      var html = svg(name, {
        size: sizeAttr ? parseInt(sizeAttr, 10) : undefined,
